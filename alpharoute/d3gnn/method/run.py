...
class run():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500, batch_size=32, vt_batch_size=32, lr=0.0005, lr_decay_factor=0.5, lr_decay_step_size=50, weight_decay=0, 
        ...
        for epoch in range(1, epochs + 1):
            ...
            valid_mae, valid_r2_score = self.val(model, valid_loader, energy_and_force, p, evaluation, device)

            print('\n\nTesting...', flush=True)
            test_mae, test_r2_score   = self.val(model, test_loader, energy_and_force, p, evaluation, device)

            print()
            print({'Train': train_mae, 'Validation': valid_mae, 'Test': test_mae})
            print({'Train': '-', 'Validation': valid_r2_score, 'Test': test_r2_score})
    ...
    def train(self, model, optimizer, train_loader, energy_and_force, p, loss_func, device):
        ...
        for step, batch_data in enumerate(tqdm(train_loader)):
            ...
            if energy_and_force:
                ...
                e_loss = loss_func(out, batch_data.y.unsqueeze(0))
                f_loss = loss_func(force, batch_data.force)
                loss = e_loss + p * f_loss
            else:

                loss = loss_func(out, batch_data.y.unsqueeze(0))
    ...
    def val(self, model, data_loader, energy_and_force, p, evaluation, device):
        ...
        for step, batch_data in enumerate(tqdm(data_loader)):
            ...
            targets = torch.cat([targets, batch_data.y.unsqueeze(0)], dim=0)
            Xs = torch.cat([Xs, batch_data.g], dim=0)
        ...
        y_true = input_dict['y_true'].cpu().numpy()
        y_pred = input_dict['y_pred'].cpu().numpy()
        ...
        for i in range(0, 5):
            plt.cla()
            plt.scatter(x, y_true[:,i], c='r', label='true')
            plt.scatter(x, y_pred[:,i], c='b', label='pred')
            plt.savefig(f'{i}.png')
        ...
        return evaluation.eval(input_dict)['mae'], evaluation.eval(input_dict)['r2_score']