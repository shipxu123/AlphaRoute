...
class HSchNet(torch.nn.Module):
    ...
    def forward(self, data):
        g = data.g
        z, pos, batch = data['ap'].nid,  data['ap'].pos, data['ap'].batch
        ...
        dist = (cost_pos[row] - cost_pos).norm(dim=-1)
        dist_emb = self.dist_emb(dist)

        # connection graph
        row_c, col_c = data['ap', 'connect','ap'].edge_index
        dist_c = ((pos[row_c] - pos[col_c]) * g).norm(dim=-1)
        ...
        for update_e, update_v in zip(self.update_es, self.update_vs):
            ...
            # connection graph
            e_c = update_e(v, dist_c, dist_emb_c, data['ap', 'connect','ap'].edge_index)
            v = update_v(v, e_c, data['ap', 'connect','ap'].edge_index)
