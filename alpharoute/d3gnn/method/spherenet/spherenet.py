...
class init(torch.nn.Module):
    def __init__(self, num_radial, hidden_channels, act=swish, use_node_features=True):
        ...
        if self.use_node_features:
            self.emb = Embedding(18, hidden_channels)
...
class SphereNet(torch.nn.Module):
    ...
    def __init__(
        ...
        hidden_channels=128, out_channels=5, int_emb_size=64,
    ...
    def forward(self, data):
        z, g, pos, batch = data.nid, data.g + 1, data.pos, data.batch
        ...
        dist, angle, torsion, i, j, idx_kj, idx_ji = xyz_to_dat(pos * g, edge_index, num_nodes, use_torsion=True)
