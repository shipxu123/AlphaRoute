...
class ThreeDEvaluator:
    ...
    def eval(self, input_dict):
        ...
        if isinstance(y_true, torch.Tensor):
            return {'mae': torch.mean(torch.abs(y_pred - y_true)).cpu().item(), 'r2_score': r2_score(y_pred, y_true).cpu().item()}
        else:
            return {'mae': float(np.mean(np.absolute(y_pred - y_true))), 'r2_score': r2_score(y_pred, y_true)}